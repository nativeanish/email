...
import { Mail, ChevronRight, Lock, Shield } from "lucide-react"
...
export default function OnBoard() {
  ...
  const [selectedOption, setSelectedOption] = useState<"arns" | "wallet" | null>(null)
  const [currentStep, setCurrentStep] = useState(1)
  const { address, walletType } = useAddress()
  const [showModal, setShowModal] = useState(false)
  useEffect(() => {
    console.log(address, walletType)
  ...
  return (
    <div>
        ...
        <div className="w-full md:w-3/5 bg-zinc-900 p-8 flex flex-col justify-between min-h-screen relative z-10">
          ...
          <div className="flex-1 flex flex-col items-center justify-center max-w-md mx-auto w-full">
            ...
            <div className="space-y-4 w-full">
              ...
              >
                <div
                  className={`absolute inset-0 bg-gradient-to-r from-white/5 to-transparent ${selectedOption === "arns" ? "opacity-100" : "opacity-0"} transition-opacity duration-500`}
                ></div>
                <div
                  className={`h-14 w-14 rounded-lg flex items-center justify-center mr-5 transition-all duration-500 ${selectedOption === "arns" ? "bg-white" : "bg-zinc-800"}`}
                >
                  <div
                    className={`h-7 w-7 transition-colors duration-500 ${selectedOption === "arns" ? "text-black" : "text-gray-400"}`}
                  >
                    <Ario theme={selectedOption === "arns" ? "dark" : "light"} />
                  </div>
                </div>
                <div className="flex-1 z-10">
                  <h3 className="font-medium text-lg">ArNS</h3>
                  <p className="text-sm text-gray-400">nativeanish@perma.email</p>
                </div>
                <motion.div
                  className="text-white"
                  initial={{ x: 10, opacity: 0 }}
                  animate={{
                    x: selectedOption === "arns" ? 0 : 10,
                    opacity: selectedOption === "arns" ? 1 : 0,
                  }}
                  transition={{ duration: 0.3 }}
                >
                ...
                <motion.div
                  ...
                  initial={{ width: "0%" }}
                  animate={{ width: selectedOption === "arns" ? "100%" : "0%" }}
                  transition={{ duration: 0.7 }}
                />
              </motion.div>

              <motion.div
                className={`border ${selectedOption === "wallet" ? "border-white" : "border-zinc-700"} 
                bg-zinc-800/50 rounded-lg p-5 flex items-center hover:bg-zinc-800 transition-all duration-300 
                cursor-pointer relative overflow-hidden group`}
              ...
              >
                <div
                  className={`absolute inset-0 bg-gradient-to-r from-white/5 to-transparent ${selectedOption === "wallet" ? "opacity-100" : "opacity-0"} transition-opacity duration-500`}
                ></div>
                <div
                  className={`h-14 w-14 rounded-lg flex items-center justify-center mr-5 transition-all duration-500 ${selectedOption === "wallet" ? "bg-white" : "bg-zinc-800"}`}
                >
                  <div
                    className={`h-7 w-7 transition-colors duration-500 ${selectedOption === "wallet" ? "text-black" : "text-gray-400"}`}
                  >
                    <Arweave theme={selectedOption === "wallet" ? "light" : "dark"} />
                  </div>
                </div>
                <div className="flex-1 z-10">
                  <h3 className="font-medium text-lg">Address</h3>
                  <p className="text-sm text-gray-400">{"TwP..._HH832E"}@perma.email</p>
                </div>
                <motion.div
                  className="text-white"
                  initial={{ x: 10, opacity: 0 }}
                  animate={{
                    x: selectedOption === "wallet" ? 0 : 10,
                    opacity: selectedOption === "wallet" ? 1 : 0,
                  }}
                  transition={{ duration: 0.3 }}
                >
                  <ChevronRight className="h-6 w-6" />
                ...
                <motion.div
                  className="absolute bottom-0 left-0 h-0.5 bg-white"
                  initial={{ width: "0%" }}
                  animate={{ width: selectedOption === "wallet" ? "100%" : "0%" }}
                  transition={{ duration: 0.7 }}
                />
              </motion.div>
            </div>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} theme="light" title="Select a Wallet to Continue">
        <Connect />
      </Modal>
    </div>
  )
